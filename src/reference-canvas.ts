...
import { HEADER_HEIGHT, fitPlacement, pinRatio, placementHeight, type Pin, type Placement, type ShelfStore } from './state';
...
export class ReferenceCanvas extends Component {
  ...
  constructor(readonly doc: Document, readonly id: string, private readonly store: ShelfStore,
    ...
    this.el = host.createDiv({ cls: 'rs-canvas', attr: { 'aria-label': 'Reference canvas' } });
    const win = doc.defaultView ?? window;
    this.registerDomEvent(win, 'resize', () => { for (const card of this.cards.values()) card.draw(); });
  }
  ...
  position(pin: Pin, x: number, y: number, width = 280): Placement {
    ...
    return fitPlacement({ x: x - rect.left, y: y - rect.top, width, collapsed: false, z: 1 }, pin, this.bounds());
  }
  ...
  render(note: string | null, tab: string | null, force = false): void {
    ...
    visible.forEach((pin, index) => {
      ...
      if (!card) {
        card = new PinCard(this.el, pin, placement, this.adapter, () => this.bounds(), this.actions(pin, placement));
  ...
  preview(pin: Pin, x: number, y: number): Placement {
    ...
    const height = placementHeight(p, pin) - HEADER_HEIGHT, width = Math.min(p.width, height * pinRatio(pin));
  ...
  stopPreview(): void { this.ghost?.remove(); this.hint?.remove(); this.ghost = null; this.hint = null; }
  toggle(): void { this.hidden = !this.hidden; this.el.classList.toggle('rs-hidden', this.hidden); }
